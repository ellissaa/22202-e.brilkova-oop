...
static int mod(int dividend, int divisor) { // остаток от деления (положит.)
...
std::set<int> Universe::default_birth_rule = {3};
std::set<int> Universe::default_survival_rule = {2, 3};

void Universe::StartField(std::set<std::pair<int, int>> coords) {
    for (int i = 0; i < m_width * m_height; i++) {
        m_field[i] = 0;
    }

    for (const auto& x_y : coords) { // обозначили живые клетки
...
Universe::Universe(std::string name, std::set<int> birth_rule, std::set<int> survival_rule,
                   std::set<std::pair<int, int>> coords) :
                   m_name(name), m_birth_rule(birth_rule), m_survival_rule(survival_rule), m_iteration_number(1) {
...
void Universe::Tick() {
    ...
    for (int y = 0; y < m_height; y++) {
        for (int x = 0; x < m_width; x++) {
            ...
            if (m_field[y * m_width + x] == 0) { // тек. клетка неживая
...
void Universe::PrintUniverse() {
    ...
    std::string cut(m_width + 2, '#'); // снизу и сверху
    std::cout << cut << "\n";
    for (int y = 0; y < m_height; y++) { // сбоку
        std::cout << '|';
        for (int x = 0; x < m_width; x++) {
