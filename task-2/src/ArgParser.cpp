...
/// @return true если num_of_iter - положительное целое число.
...
std::set<std::string> ArgParser::valid_args{
...
void ArgParser::ParseArgs(int argc, char** argv) {
    for (int i = 1; i < argc; i++) {
        ...
        if (arg_map.contains(short_name)) { // если встретился аргумент 2 раза
...
std::map<std::string, std::string> ArgParser::GetArgs() {
    return arg_map;
}
